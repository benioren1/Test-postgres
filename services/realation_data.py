...
def normal_database():
    ...
    try:
        ...
        while True:
            ...
            aircraft_returned = mission_row[26]
            aircraft_failed = mission_row[27]
            aircraft_damaged = mission_row[28]
            aircraft_lost = mission_row[29]
            high_explosives = mission_row[30]
            high_explosives_type = mission_row[31]
            high_explosives_weight_pounds = mission_row[32]
            high_explosives_weight_tons = mission_row[33]
            incendiary_devices = mission_row[34]
            incendiary_devices_type = mission_row[35]
            incendiary_devices_weight_pounds = mission_row[36]
            incendiary_devices_weight_tons = mission_row[37]
            fragmentation_devices = mission_row[38]
            fragmentation_devices_type = mission_row[39]
            fragmentation_devices_weight_pounds = mission_row[40]
            fragmentation_devices_weight_tons = mission_row[41]
            total_weight_pounds = mission_row[42]
            total_weight_tons = mission_row[43]
            time_over_target = mission_row[44]
            bomb_damage_assessment = mission_row[45]
            source_id = mission_row[::-1]

            insert_country = '''
                            INSERT INTO country (country_name) VALUES (%s) RETURNING country_id;
                        '''
            s_old.execute("SELECT country_id FROM country WHERE country_name = %s", (target_country,))
            existing_country = s_old.fetchone()

            if existing_country:
                country_id1 = existing_country[0]
            else:
            ...
            insert_target = f'''
                insert into targets (target_key, target_type, target_industry, target_priority, loc_id) VALUES (%s, %s, %s, %s, %s)
                 returning target_id,(target_key,target_type, target_industry, target_priority, {loc_id1})
            '''
            print(f"Inserting target: {target_key}, {target_type}, {target_industry}, {target_priority}, {loc_id1}")
            cur.execute(insert_target, (target_key, target_type, target_industry, target_priority, loc_id1))
            target_id1 = cur.fetchone()[0]

            insert_mission = f'''
                insert into mission (mission_date, theater_of_operations, country, air_force, unit_id, aircraft_series,
                                    callsign, mission_type, takeoff_date, landing_date,
                                    takeoff_base, takeoff_location, takeoff_latitude, takeoff_longitude, target_key,
                                    target_id, altitude_hundreds_of_feet, airborne_aircraft, attacking_aircraft, bombing_aircraft,
                                    aircraft_returned, aircraft_failed, aircraft_damaged, aircraft_lost, high_explosives,
                                    ...
                                    total_weight_pounds, total_weight_tons, time_over_target, bomb_damage_assessment, source_id)
                                     values (%s, %s, %s, %s, %s, %s, %s, %s,
                                            ...
                                            (mission_date, theater_of_operations, country, air_force, unit_id, aircraft_series,
                                             callsign, mission_type, takeoff_date, landing_date,
                                            takeoff_base, takeoff_location, takeoff_latitude, takeoff_longitude, target_key,
                                            {target_id1}, altitude_hundreds_of_feet, airborne_aircraft, attacking_aircraft, bombing_aircraft,
                                            aircraft_returned, aircraft_failed, aircraft_damaged, aircraft_lost, high_explosives,
                                            high_explosives_type, high_explosives_weight_pounds, high_explosives_weight_tons, incendiary_devices,
                                            incendiary_devices_type, incendiary_devices_weight_pounds, incendiary_devices_weight_tons,
                                            fragmentation_devices, fragmentation_devices_type, fragmentation_devices_weight_pounds, fragmentation_devices_weight_tons,
                                            total_weight_pounds, total_weight_tons, time_over_target, bomb_damage_assessment, source_id)'''
            print(f"Inserting mission: {mission_id}")
            cur.execute(insert_mission, (mission_date, theater_of_operations, country, air_force, unit_id, aircraft_series,))
            conn_new.commit()
